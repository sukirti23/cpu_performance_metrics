...
# e.g., ./h264dec.sh perf-result
# this script also creates a csv file and appends the required output data in that csv file.
# This script also creates a total_output_h246.txt  where the perf results are appended to verify csv output file and deletes the original argument file.

csvFile="results_h246dec.csv"
...
# The required number of threads and cores are passed to this function by 2 for loops following the function.
...
perf_inst()
{
    ...
    for j in 1 2 3
    do
    	perf stat -e instructions,cycles -o  $outputFile --append taskset -c $cores ./h264dec -i ./park_joy_2160p.h264 -t $threads
	ins_per_cyc=`cat $outputFile | grep insn | awk '{print $4}'`
	exe_time=`cat $outputFile | grep elapsed | awk '{print $1}' | sed 's/,//g'`
	./a.out
	perf stat -e L1-dcache-loads,L1-dcache-load-misses -o $outputFile  --append taskset -c $cores ./h264dec -i ./park_joy_2160p.h264 -t $threads
	...
	./a.out
	perf stat -e rE124,r2124 -o $outputFile --append taskset -c $cores   ./h264dec -i ./park_joy_2160p.h264 -t $threads
	...
	./a.out	
	perf stat -e LLC-loads,LLC-load-misses -o $outputFile --append taskset -c $cores   ./h264dec -i ./park_joy_2160p.h264 -t $threads	
	...
	cat $outputFile >> total_output_h246.txt
	rm -f $outputFile
    done 
